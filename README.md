...
**Two orthogonal properties — don't conflate them.** *Redundancy* and *crash-proofness* are separate here, bought by separate mechanisms, and only one is the unusual one.

- **Redundancy** — surviving a bad block or a dead device — is what the two mirror rings buy, and it rests on the founding trade: *flash is cheap, correctness is not.* Two full rings + write-then-read-back-verify + relocate-don't-repair is ~2× flash, ~2× write bandwidth, ~2× write power, spending the abundant resource to insure the scarce one. On commodity flash that is nearly free. But this part is **old** — RAID-1, ZFS `copies=2` (mirroring), T10-PI (verify-on-write), ZFS/btrfs (content-seal + COW). The only twist is the *altitude*: running it unprivileged, per-app, in userspace on one phone-class device an FTL already manages, instead of in the kernel or a volume manager.

- **Crash-proofness** — a kill at *any* byte boundary leaves the store committed-or-absent, deterministically, with **no recovery pass** — is a property of the *commit protocol*, not the redundancy. It would hold on a **single** ring with no mirror at all. The spine entry is the atomic transaction point; generations + the rollback fence + heartbeat give "put `0..G` intact, put `G` fully absent, never partial." There is no journal, no fsck, no separate recovery code, because **the recovery path *is* the open path**, run on every boot. The kill harness is the proof, not a slogan: kill -9 at every byte offset across a write, reopen, and it comes up correct **100% of the time, every time** — plus the pinned counter-example that a single corrupt spine slot can't deflect the head-search bisect to a stale generation.

Redundancy protects against the medium; crash-proofness protects against the interruption. RAID gives you the first and says nothing about the second (it will faithfully mirror a torn write). The uncommon thing here is the **second, done with zero recovery code and pinned by an exhaustive kill harness** — a commit-protocol achievement, independent of and additive to the mirror. Neither is a new primitive on its own; the combination, at this altitude, is where the novelty (if any) lives — a claim worth a real prior-art search before it's staked, not asserted.
...
A 15ms power cut, a cosmic-ray bit flip, and a tampered byte all produce the identical symptom — a block whose seal fails — and receive the identical treatment: classify Corrupt.
...
> **Detection is built; per-block inline heal is not yet — see "Bad-block relocation" under "What this is not (yet)".** Today a corrupt block is detected (seal fails) and a whole stale/missing ring is reconciled by explicit `verified_replicate`, but the read path does not yet serve a corrupt block's good sibling copy inline or repair the bad one. The intended design is relocation, not repair (below).
...
- **Bad-block relocation is designed, not built — and splits on the storage profile.** Today `Mirror::read` routes to the first present device and does not consult the sibling when a block reads Corrupt; healing a bad block requires an explicit whole-ring `verified_replicate`. The intended fix is **relocation, not repair** — deliberately, because repair means reconciliation (block N differs between rings: which is truth, rewrite A from B or B from A, what about a mid-torn-write, what about generations) and that is a hairball. Relocation never creates two competing versions of one logical block, so there is nothing to reconcile. The mechanics differ by whether something below us already remaps bad blocks:

  **Host profile (managed flash — phones, SSDs; the case today).** The FTL under us already does wear-leveling + bad-block remapping, and owns the physical truth. So we stay **stateless**: write → verify → on failure relocate forward and rewrite the ref (tract) or burn a generation (ring), and just re-try the sector on the next pass — a slot that failed once may well succeed next time because the FTL remapped it. No tombstone map of our own; the layer below is the map.

  **Kernel profile (raw NAND we manage ourselves — the ferros destination).** Now *we* are the FTL: no layer below remaps, so a bad block stays bad forever. Here the tombstone earns its keep — a **bad-block tombstone** (distinct from the deleted-zero: zero = free/deleted; a bad-block marker = "never hand this slot out again"). Both paths **read/check-for-tombstone, then write/verify**, so a persistent bad slot isn't re-paid every pass. *Only if* that per-write check proves too costly at scale does an explicit bad-block **map** (or a RAM cache rebuilt at open, like `TODO(scale)`) become worth its complexity — not before.

  Common to both, and to both the tract and the ring:
  - A block that fails write-verify (or is found Corrupt on read) is **not fought**. The data lands elsewhere: the tract's plow writes the **next free block** (free-choice address); the ring **burns a generation** — slot is `gen mod N` by construction ([ring.rs](src/ring.rs)), so G's slot can't be reused for other data; commit as G+1, which lands at a *different* physical slot, and the bisect already branches around the abandoned/Corrupt slot ([README head-search](#) — "an ambiguous read prunes nothing").
  - Both rings **lock-step relocate** and stay byte-identical (a block address means the same thing on both rings — the invariant read-repair leans on). A bad block on *either* ring burns that slot on *both*; a 4KB slot / one generation is a cheap price for the invariant, and bad blocks are rare.
  - The **index/extent re-point (tract) or the new spine entry (ring) is the commit**; any tombstone is advisory. If a tombstone write to bad media *also* fails, that is more confirmation, not a problem — the orphaned block is dead the moment the reference moved off it.

  This is the append-only/COW grain the store already has (the plow writes forward; the ring's generation counter has no ceiling), and it composes with the reap (a dead/tombstoned block is another slot to retire). Tracked as `TODO(read-repair)` in `src/mirror.rs`. Needed before the calls/attachments layer, where one bad block in a multi-thousand-block recording must self-heal rather than surface as Corrupt.
